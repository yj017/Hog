...
def boar_brawl(player_score, opponent_score):
    ...
    opsc=opponent_score//10%10
    psc=player_score%10

    return max(1, 3*abs(opsc-psc))
    # END PROBLEM 2
...
def is_always_roll(strategy, goal=GOAL):
    ...
    score1=0
    score2=0
    num=strategy(score1, score2)
    while score1<=goal-1:
        score2=0
        while score2<=goal-1:
            if(strategy(score1, score2)!=num):
                return False
            score2=score2+1
        score1=score1+1
    return True
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    for num_rolls in range(1, 11):
        curr_avg = make_averaged(roll_dice, samples_count)(num_rolls, dice)
        if curr_avg > max_avg:
            max_avg = curr_avg
